...
class Net(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        hidden_nodes1 = 1024
        hidden_nodes2 = 512
        self.fc1 = nn.Linear(4 * 3 * 11, hidden_nodes1)
        ...
        for epoch in range(int(num_epochs)):
            ...
            if epoch % period == 0:
                ...
                try:
                    ...
                    pass
                np.save(
                    self.timestamp + "/double_dqn_cnn_intersect_loss.npy", loss_list
                )
                np.save(
                    self.timestamp + "/double_dqn_cnn_intersect_avg_reward.npy",
                    avg_reward_list,
                )
                torch.save(
                    self.estimate_net.state_dict(),
                    self.timestamp + "/double_dqn_cnn_intersect.pkl",
                )
...
if __name__ == "__main__":
    ...
    np.save(time_string + "/double_dqn_cnn_intersect_loss.npy", avg_loss)
    np.save(time_string + "/double_dqn_cnn_intersect_avg_reward.npy", avg_reward_list)

    # save the dqn network
    torch.save(
        double_dqn_cnn_object.estimate_net.state_dict(),
        time_string + "/double_dqn_cnn_intersect.pkl",
    )
